...
public class Search {
	...
	public void equalSearch() throws IOException, InterruptedException, JSchException {
		if (objectClass.equals(ObjectClass.ACCOUNT)) {
			PasswdFile passwdFile = generalSearch();
	...
	private PasswdFile generalSearch() throws JSchException, IOException {
		UnixResult result = unixConnection.execute(UnixConnector.getCommandGenerator().searchAllUser());
		result.checkResult(Operation.GETENET);
		PasswdFile passwdFile = new PasswdFile(getPasswdFileOutput(result.getOutput()));
		return passwdFile;
	}
	...
	private void fillGroupHandler() throws IOException, InterruptedException, JSchException {
		...
		if (StringUtil.isNotBlank(nameToSearch)
				...
				&& EvaluateCommandsResultOutput.evaluateUserOrGroupExists(unixConnection.execute(
						UnixConnector.getCommandGenerator().groupExists(nameToSearch)).getOutput())) {
			ConnectorObjectBuilder bld = new ConnectorObjectBuilder();
